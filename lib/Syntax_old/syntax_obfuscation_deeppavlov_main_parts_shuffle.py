...
import re
import random
class Syntax_deeppavlov_main_parts_shuffle(object):
    def __init__(self):
    ...
    def part_shuffler(self,sentence):
        ...
        for i,tree_elems in enumerate(syntax_tree):
            if tree_elems[3] == 'root' and int(tree_elems[2]) == -1:
                root = i
        for i,tree_elems in enumerate(syntax_tree):
            if tree_elems[3] == 'nsubj' and int(tree_elems[2]) == root :
                nsubj = i
    ...
    def find_childrens_rs(self,root,nsubj,syntax_tree):
        ...
        while search:
            get_one = False
            for tree_elems in syntax_tree:
                if (tree_elems[2] in branches_root and tree_elems[0] not in used_root and tree_elems[0] != root and tree_elems[2] != root):
                    get_one = True
                    childrens_root[tree_elems[2]].append(tree_elems[0])
                    used_root.append(tree_elems[0])
                elif tree_elems[0] not in used_root and tree_elems[0] != root and tree_elems[2] != root:
                    for branch in branches_root:
                        if tree_elems[2] in childrens_root[branch]:
                            get_one = True
                            childrens_root[branch].append(tree_elems[0])
                            used_root.append(tree_elems[0]) 
                elif (tree_elems[2] in branches_nsubj and tree_elems[0] not in used_nsubj and tree_elems[0] != root and tree_elems[2] != root):
                    get_one = True
                    childrens_nsubj[tree_elems[2]].append(tree_elems[0])
                    used_nsubj.append(tree_elems[0])
                elif tree_elems[0] not in used_nsubj and tree_elems[0] != root and tree_elems[2] != root:
                    for branch in branches_nsubj:
                        if tree_elems[2] in childrens_nsubj[branch]:
                            get_one = True
                            childrens_nsubj[branch].append(tree_elems[0])
                            used_nsubj.append(tree_elems[0])  
            if not get_one:
                search = False
    ...
    def find_branches_rs(self,root,nsubj,syntax_tree):
        ...
        branches_root = []
        for tree_elems in syntax_tree:
            if tree_elems[2] == root and tree_elems[0] != nsubj:
                branches_root.append(tree_elems[0])
                
        branches_nsubj = []
        for tree_elems in syntax_tree:
            if tree_elems[2] == nsubj and tree_elems[0] != root:
                branches_nsubj.append(tree_elems[0])
